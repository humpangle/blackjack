defmodule Blackjack.RoundTest do
  ...
  defmacrop notify_player(player_id, data) do
    quote do
  ...
  test "initial play" do
    ...
    assert deals == [
             notify_player(:a, {:deal_card, card(2)}),
             notify_player(:a, {:deal_card, card(3)}),
             notify_player(:a, :move)
           ]
  end
  ...
  test "stand moves to next player" do
    ...
    assert Enum.member?(instructions, notify_player(:b, :move))

    assert {instructions, round} = Round.move(round, :b, :stand)
    assert Enum.member?(instructions, notify_player(:c, :move))

    assert {instructions, _} = Round.move(round, :c, :stand)
    assert Enum.any?(instructions, &match?(notify_player(:a, {:winners, _}), &1))
  end
  ...
  test "taking a hit" do
    ...
    assert {
             [
               notify_player(:a, {:deal_card, card}),
               notify_player(:a, :move)
             ],
             _round
           } = Round.move(round, :a, :hit)
  ...
  test "busting" do
    ...
    assert Enum.member?(instructions, notify_player(:a, :busted))
    assert Enum.member?(instructions, notify_player(:a, {:winners, []}))
  end
  ...
  test "one winner" do
    ...
    assert Enum.member?(instructions, notify_player(:a, {:winners, [:b]}))
  end
  ...
  test "multiple winners" do
    ...
    assert {[
              notify_player(:a, {:winners, [:a, :b]}),
              notify_player(:b, {:winners, [:a, :b]}),
              notify_player(:c, {:winners, [:a, :b]})
  ...
  test "unauthorized move" do
    {_, round} = Round.start([:a, :b, :c])
    assert {instructions, _} = Round.move(round, :b, :stand)
    assert Enum.member?(instructions, notify_player(:b, :unauthorized_move))
  end
  ...
  test "more players than cards in a single deck" do
    ...
    assert Enum.any?(instructions, &match?(notify_player(_, {:winners, _}), &1))
  end
