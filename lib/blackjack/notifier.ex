defmodule Blackjack.PlayerNotifier do
  ...
  def publish(round_id, player_id, player_instruction),
    do: GenServer.cast(service_name(round_id, player_id), {:notify, player_instruction})

  @doc false
  def start_link(round_id, player),
    do:
  ...
  @doc false
  def init({round_id, player}), do: {:ok, %{round_id: round_id, player: player}}
  ...
  defp service_name(round_id, player_id),
    do: Blackjack.service_name({__MODULE__, round_id, player_id})
